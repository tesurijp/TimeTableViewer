...
function loadData(data)
{
    ...
    t = 0 ;
    cnt = 0 ;
    this[t] = { data:new Array(), hour:new Array(), name:""} ;
...
function loadHoliday(data) 
{
    ...
    {   
        ...
        {
            this[this.length] = { y:RegExp.$1, m:RegExp.$2, d:RegExp.$3, c:RegExp.$4 } ;
            this.length += 1 ;
        }
        else if (/([0-9]+)\/([0-9]+):(.*)/.test(line[row])) 
        {
            this[this.length] = { y:0, m:RegExp.$1, d:RegExp.$2, c:RegExp.$3 } ;
            this.length += 1 ;
        }
    }
...
function loadMenu(data) 
{
    ...
    for (row=0; row<line.length; row++)
    {
        ...
        {
            line[row].match(/(\w+),(.*)/) ;
            this[this.length] = { ID:RegExp.$1, string:RegExp.$2 } ;
            this.length += 1 ;
        }
    }
    ...
    for (row=first; row<table.hour.length && count < max; row++)
    {
        ...
        for (col=0; col<token.length && count < max; col++)
        {
            ...
            else
            {
                if (isMatch)
                {
                    this[count] = { hour:Number(table.hour[row]), min:Number(token[col]), comment:string } ;
                    count += 1 ;
                }   
                else if (filter.source == ".*")
                {
                    this[count] = { hour:Number(table.hour[row]), min:Number(token[col]), comment:'' } ;
                    count += 1 ;
                }
                isMatch = false ;
                string = '' ;
...
//      t = getTableByDate(today) ;
    ...
    for (row=table.hour.length-1; row>=0 && count < max; row--)
    {
        ...
        for (col=token.length-1; col>=0 && count < max; col--)
        {
            ...
            if (filter.source != ".*")
            {
                ...
                if (filter.test(string))
                {
                    work[count] = { hour:Number(table.hour[row]), min:Number(min[col]), comment:string } ;
                    count += 1 ;
                }   
            }
            else
    ...
    for (loop=0; loop<count; loop++)
        this[loop] = { hour:work[loop].hour, min:work[loop].min, comment:work[loop].comment } ;
...
function getAllList(method, filter) 
{
    ...
    if (method == 'sun') 
    {
        ...
        this.dateString = "日曜日の時刻表" ;
    }
    else if (method == 'mon') 
    {
        var t = TableData.week[1] ;
        this.dateString = "月曜日の時刻表" ;
    }
    else if (method == 'tue') 
    {
        var t = TableData.week[2] ;
        this.dateString = "火曜日の時刻表" ;
    }
    else if (method == 'wed') 
    {
        var t = TableData.week[3] ;
        this.dateString = "水曜日の時刻表" ;
    }
    else if (method == 'thu') 
    {
        var t = TableData.week[4] ;
        this.dateString = "木曜日の時刻表" ;
    }
    else if (method == 'fri') 
    {
        var t = TableData.week[5] ;
        this.dateString = "金曜日の時刻表" ;
    }
    else if (method == 'sat') 
    {
        var t = TableData.week[6] ;
        this.dateString = "土曜日の時刻表" ;
    }
    else if (method == 'hol') 
    {
        var t = TableData.week[7] ;
        this.dateString = "祝日の時刻表" ;
    }
    else
        var t = 0 ;
